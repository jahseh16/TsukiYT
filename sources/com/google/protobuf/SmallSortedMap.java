...
class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    */
    ...
    public V put(K k3, V v6) {
        checkMutable();
        int iBinarySearchInArray = binarySearchInArray(k3);
        if (iBinarySearchInArray >= 0) {
        ...
        if (i >= this.maxArraySize) {
            return getOverflowEntriesMutable().put(k3, v6);
        }
        int size = this.entryList.size();
        int i5 = this.maxArraySize;
        if (size == i5) {
            SmallSortedMap<K, V>.Entry entryRemove = this.entryList.remove(i5 - 1);
            getOverflowEntriesMutable().put(entryRemove.getKey(), entryRemove.getValue());
        }
        this.entryList.add(i, new Entry(k3, v6));
        return null;
    }
}
