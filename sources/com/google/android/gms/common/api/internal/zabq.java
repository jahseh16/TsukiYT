...
public final class zabq implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, zau {
    ...
    private final Feature zaC(@Nullable Feature[] featureArr) {
        if (featureArr != null && featureArr.length != 0) {
            ...
            y yVar = new y(availableFeatures.length);
            for (Feature feature : availableFeatures) {
                yVar.put(feature.getName(), Long.valueOf(feature.getVersion()));
            }
            for (Feature feature2 : featureArr) {
                Long l3 = (Long) yVar.get(feature2.getName());
    ...
    private final boolean zaM(zai zaiVar) {
        ...
        zabs zabsVar = new zabs(this.zad, featureZaC, (zabr) null);
    ...
    public static /* bridge */ /* synthetic */ void zam(zabq zabqVar, zabs zabsVar) {
        ...
        if (zabqVar.zak.remove(zabsVar)) {
            ...
            for (zac zacVar : zabqVar.zab) {
                if ((zacVar instanceof zac) && (featureArrZab = zacVar.zab(zabqVar)) != null && ArrayUtils.contains(featureArrZab, featureZaa)) {
            ...
            for (int i = 0; i < size; i++) {
                zai zaiVar = (zai) arrayList.get(i);
                zabqVar.zab.remove(zaiVar);
                zaiVar.zae(new UnsupportedApiCallException(featureZaa));
            }
        }
    }
