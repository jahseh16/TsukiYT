...
public interface zn {
    ...
    List<n3> getAllEligibleWorkSpecsForScheduling(int i);
    ...
    @Query("SELECT id FROM workspec")
    @Transaction
    xc<List<String>> getAllWorkSpecIdsLiveData();
    ...
    @Transaction
    List<n3.zn> getWorkStatusPojoForName(String str);

    @Query("SELECT id, state, output, run_attempt_count FROM workspec WHERE id IN (SELECT work_spec_id FROM worktag WHERE tag=:tag)")
    @Transaction
    List<n3.zn> getWorkStatusPojoForTag(String str);

    @Query("SELECT id, state, output, run_attempt_count FROM workspec WHERE id IN (:ids)")
    @Transaction
    xc<List<n3.zn>> getWorkStatusPojoLiveDataForIds(List<String> list);

    @Query("SELECT id, state, output, run_attempt_count FROM workspec WHERE id IN (SELECT work_spec_id FROM workname WHERE name=:name)")
    @Transaction
    xc<List<n3.zn>> getWorkStatusPojoLiveDataForName(String str);

    @Query("SELECT id, state, output, run_attempt_count FROM workspec WHERE id IN (SELECT work_spec_id FROM worktag WHERE tag=:tag)")
    @Transaction
    xc<List<n3.zn>> getWorkStatusPojoLiveDataForTag(String str);
    ...
    void setOutput(String str, androidx.work.n3 n3Var);
    ...
    int setState(z.y yVar, String... strArr);
}
